...
import {
  ...
  DollarSign, ShoppingBag, BarChart3, Star, ArrowRight,
} from 'lucide-react';
    ...
    <div>
      ...
      <div className="card p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-base">Pedidos en vivo</h2>
          <span className="text-xs text-gray-400">Últimos {latestOrders.length}</span>
        </div>
        {latestOrders.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-8">Todavía no recibiste pedidos</p>
        ) : (
            ...
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="text-left text-[11px] text-gray-400 uppercase tracking-wider border-b border-neutral-100">
                  ...
                  <th className="px-5 py-2 font-semibold">Productos</th>
                  <th className="px-5 py-2 font-semibold">Cliente</th>
                  <th className="px-5 py-2 font-semibold">Tiempo</th>
                  <th className="px-5 py-2 font-semibold">Acciones</th>
                </tr>
              </thead>
              <tbody>
                {latestOrders.map(order => {
                  ...
                  return (
                    <tr key={order.id} className="border-b border-neutral-50 last:border-0">
                      <td className="px-5 py-3 font-mono text-xs text-gray-400">#{order.id.slice(0, 6).toUpperCase()}</td>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          <span className={`badge ${statusInfo.color} shrink-0`}><StatusIcon size={12} /> {statusInfo.label}</span>
                          {order.order_status === 'pending' && (
                            <>
                              <button onClick={() => updateStatus(order.id, 'accepted')} title="Aceptar" className="w-7 h-7 flex items-center justify-center rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition-colors">
                                <CheckCircle size={14} />
                              </button>
                              <button onClick={() => updateStatus(order.id, 'rejected')} title="Rechazar" className="w-7 h-7 flex items-center justify-center rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition-colors">
                                <XCircle size={14} />
                              </button>
                            </>
                          )}
                          {NEXT_STATUS[order.order_status] && (
                            <button
                              ...
                              title={`Marcar como ${STATUS_LABELS[NEXT_STATUS[order.order_status]]?.label}`}
                              className="w-7 h-7 flex items-center justify-center rounded-lg bg-primary-bg text-primary hover:bg-primary/10 transition-colors"
                            >
                              <ArrowRight size={14} />
                            </button>
                          )}
      ...
      {/* Todos los pedidos */}
      <h2 className="font-bold text-base mb-3">Todos los pedidos</h2>

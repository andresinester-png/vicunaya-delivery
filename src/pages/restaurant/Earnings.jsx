        ...
        <div className="flex gap-2">
          {[['today','Hoy'],['week','7 días'],['month','Este mes']].map(([key, label]) => (
            <button key={key} onClick={() => setRange(key)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${range === key ? 'bg-primary text-white' : 'bg-white text-gray-600 border border-neutral-200'}`}>
              {label}
            </button>
          ))}
        </div>
          ...
          {dailyRevenue.map((day, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              ...
              <div className="w-full rounded-t-lg bg-primary-bg overflow-hidden" style={{ height: '80px' }}>
                <div className="bg-primary rounded-t-lg w-full transition-all duration-500"
          ...
          {filtered.length === 0 ? (
            ...
            <div key={order.id} className="flex items-center justify-between py-2 border-b border-neutral-100 last:border-0">
              <div>
                ...
                <p className="text-xs text-gray-400">{new Date(order.created_at).toLocaleDateString('es-AR')} · {PAYMENT_LABELS[order.payment_method]}</p>
              </div>
